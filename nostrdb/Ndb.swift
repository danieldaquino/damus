...
class Ndb {
    ...
    static func open(path: String? = nil, owns_db_file: Bool = true, callbackHandler: Ndb.CallbackHandler) -> ndb_t? {
        ...
        guard owns_db_file || Self.db_files_exist(path: path) else {
    ...
    private static func migrate_db_location_if_needed() throws {
        ...
        guard let legacy_path = Self.legacy_db_path else {
            // If we cannot even get the legacy DB path, migrating is not possible
            return
        }
        
        let private_db_files_exist = Self.db_files_exist(path: db_path)
        let legacy_db_files_exist = Self.db_files_exist(path: legacy_path)
        
        guard private_db_files_exist || legacy_db_files_exist else { return }
        
        guard let latest_path = Self.latestDatabasePath(primaryPath: db_path,
                                                        legacyPath: legacy_path,
                                                        fileManager: fileManager) else { return }
        
        guard latest_path != db_path else { return }
        ...
        do {
            ...
            try db_files.forEach { db_file in
                let source = "\(legacy_path)/\(db_file)"
                let destination = "\(db_path)/\(db_file)"
