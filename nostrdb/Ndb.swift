...
class Ndb {
    ...
    /// Subscribes to NostrDB using `ndb_filter`
    func subscribe(filters: [ndb_filter], maxSimultaneousResults: Int = 1000) throws(NdbStreamError) -> AsyncStream<StreamItem> {
        var filters = filters
        ...
        guard ndb_query(&txn.txn, &filters, Int32(filters.count), results, Int32(maxSimultaneousResults), count) == 1 else {
            throw .initialQueryFailed
        }
        
        return AsyncStream<StreamItem> { continuation in
            var streaming = true
            Log.debug("ndb streaming: Started streaming", for: .ndb)
            continuation.onTermination = { @Sendable _ in
                Log.debug("ndb streaming: Terminated early before eose", for: .ndb)
                streaming = false
                count.deallocate()
                results.deallocate()
            }
            ...
            // Stream new results
            let subscriptionId = ndb_subscribe(self.ndb.ndb, &filters, Int32(filters.count))
            
            // Set the subscription callback so that we get data back from the subscription
            self.setCallback(for: subscriptionId, callback: { noteKey in
                continuation.yield(.event(noteKey))
            })
            
            // On termination, cancel the subscription and cleanup.
            continuation.onTermination = { @Sendable _ in
                Log.debug("ndb streaming: Terminated", for: .ndb)
                streaming = false
                ndb_unsubscribe(self.ndb.ndb, subscriptionId)
                self.unsetCallback(subscriptionId: subscriptionId)
                count.deallocate()
                results.deallocate()
            }
        }
