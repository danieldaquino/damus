...
final class RelayConnection: ObservableObject {
    ...
    func getMissingIds(filters: [NostrFilter], negentropyVector: NegentropyStorageVector, timeout: Duration?) async throws -> [Id] {
        ...
        var negentropyClient = try Negentropy(storage: negentropyVector, frameSizeLimit: 60_000)
        let initialMessage = try negentropyClient.initiate()
        let subscriptionId = UUID().uuidString
        for await response in negentropyStream(subscriptionId: subscriptionId, filters: filters, initialMessage: initialMessage) {
            switch response {
