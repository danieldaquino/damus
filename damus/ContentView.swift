...
struct ContentView: View {
        ...
        .onChange(of: scenePhase) { (phase: ScenePhase) in
            ...
            case .background:
                ...
                damusClosingTask = Task { @MainActor in
                    ...
                    if let appDelegate = self.appDelegate, let snapshotManager = appDelegate.snapshotManager {
                        await snapshotManager.stopPeriodicSnapshots()
                    }
    ...
    func connect() async {
        ...
        // Initialize and store snapshot manager in AppDelegate
        if let appDelegate = self.appDelegate {
            appDelegate.state = self.damus_state
            let snapshotManager = DatabaseSnapshotManager(ndb: ndb)
