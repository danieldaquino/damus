        ...
        case .nwc(let nwc_state):
            ...
            if !damus_state.settings.nozaps && !is_custom && damus_state.settings.donation_percent > 0 {
                flusher = .once({ pe in
                    ...
                    Task { @MainActor in
                        await send_donation_zap(damus_state: damus_state, postbox: damus_state.nostrNetwork.postbox, nwc: nwc_state.url, percent: damus_state.settings.donation_percent, base_msats: amount_msat)
                    }
                })
            }
            ...
            let nwc_req = await damus_state.nostrNetwork.nwcPay(url: nwc_state.url, post: damus_state.networkManager.postbox, invoice: inv, delay: delay, on_flush: flusher)
